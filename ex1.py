...
class Spell_Checker:
    ...
    def spell_check(self, text, alpha, k=1):
        ...
        Attempt context + error based correction on each word, while prioritizing sentences
        with fewer OOV words.

        Args:
            text (str): The text to spell check.
            alpha (float): The probability of keeping a lexical word as is.
            k (int): Max number of words to correct. Will be handled with a loop,
        ...
        for _ in range(k):
            ...
            for i, word in enumerate(words):
                ...
                # Correction will be a word found in the lm.vocabulary, or the original, if no candidate was found
    ...
    def _candidates(self, word):
        ...
        Args:
            word (str): A word for replacement.

        Return: A Set() of possible candidates for corrections + the original word.
        """
        candidates = self._known([word]) | self._known(self._edits1(word)) | self._known(self._edits2(word))

        # Ensure the original word is included as a fallback candidate if no other candidates are found.
    ...
    def _edits1(self, word):
        """
        All edits (not necessary in language) that are one edit away from `word`.
    ...
    def _edits2(self, word):
        """
        All edits (not necessary in language) that are 2 edits away from `word`.
    ...
    def _error_probability(self, candidate, word, alpha, min_prob=1e-8):
        ...
        for intermediate in self._edits1(word):
            ...
            for edit1 in self._edits1(intermediate):
                if edit1 == candidate:
                    # Loop to find the edit that will turn intermediate to candidate, calculate just for it.
    ...
    class Language_Model:
        ...
        def __init__(self, n=3, chars=False):
            ...
            self.model_dict = defaultdict(
                Counter)  # a dictionary of the form {ngram (tup):count}, holding counts of all ngrams in the specified text.
        ...
        def build_model(self, text):
            ...
            for i in range(len(text) - 1):
                ...
                pair = text[i:i + 2]
                self.pairs[pair] += 1
            self.singulars[text[-1]] += 1  # To account for the last character
        ...
        def smooth(self, ngram, k=1):
            ...
            based on self model.
            For an attempt to get probability for len(text) < window_size, p = k/|V|
            ...
            """
            vocabulary_size = len(self.vocabulary)  # num of unique words in vocabulary
...
def normalize_text(text, lower=True, remove_numbers=True, remove_stopwords=False):
    ...
    Args:
        ...
        lower (bool): Whether to lower the text. Defaults to True.
        remove_numbers (bool): Whether to remove numbers or dates from the text. Defaults to True.
        remove_stopwords (bool): Whether to remove stopwords from the original text, and keep only meaningful words.
                                Defaults to False, for context and negations. Whether True or False you'll also remove
    ...
    replacements = {
        '“': '"', '”': '"', '‘': "'", '’': "'",
        'â\x80\x9c': '"', 'â\x80\x9d': '"', 'â\x80\x99': "'", 'â\x80\x98': "'"
    }
    ...
    if remove_numbers:
        text = re.sub(r'\b(\d{1,4}[/-]\d{1,2}[/-]\d{1,4})\b', 'DATE', text)  # Dates like 01/01/2020 or 2020-01-01
        text = re.sub(r'\b\d+\.\d+%?\b', 'DECIMAL', text)  # Decimal numbers with optional percent sign
        text = re.sub(r'\b\d+%?\b', 'NUM', text)  # Whole numbers with optional percent sign
    ...
    if remove_stopwords:
        ...
        # remove both
        text = ' '.join([word.strip(string.punctuation) for word in text.split() if word not in stop_words])
    else:
...
def who_am_i():  # this is not a class method
    ...
    """
    return {'name': 'Alon Fridental', 'id': '209370774', 'email': 'alonfri@post.bgu.ac.il'}
